...
int traverse_bvh(float* t_min, float* t_max, intersection_point* ip, bvh_node* node, 
				vec3 ray_origin, vec3 ray_direction, float* t0, float* t1) 
{
	...
	if (bbox_intersect(t_min, t_max, left_node->bbox, ray_origin, ray_direction, *t0, *t1)) {
		traverse_bvh(t_min, t_max, ip, left_node, ray_origin, ray_direction, t0, t1);
	}
	if (bbox_intersect(t_min, t_max, right_node->bbox, ray_origin, ray_direction, *t0, *t1)) {
		traverse_bvh(t_min, t_max, ip, right_node, ray_origin, ray_direction, t0, t1);
	}
									...
									vec3 ray_direction)
{
    ...
    traverse_bvh(&t_min, &t_max, ip, bvh_root, ray_origin, ray_direction, &t0, &t1);
